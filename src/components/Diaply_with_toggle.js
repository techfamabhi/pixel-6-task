...
function Display() {
    ...
    <div className="accordion-item">
        <h2 className="accordion-header" id="panelsStayOpen-headingOne">
            <div className={`row ${currentPage === 1 ? 'active' : ''}`} style={{ cursor: 'pointer' }} onClick={() => {
                const collapseElement = document.getElementById('panelsStayOpen-collapseOne');
                collapseElement.classList.toggle('show');
            }}>
                <div className="col-md-11">
                    <img src="pixel6-logo.jpg" className="img-fluid mx-auto img1" alt=""/>
                </div>
                <div className="col-md-1 ">
                    <i className="fa fa-bars mx-auto bar-icon"></i>
                </div>
            </div>
        </h2>
    ...
    <DataTable
        ...
        paginationTotalRows={filteredUsers.length} // Total rows based on filtered data
        progressPending={isLoading} // Show loading indicator
        onChangePage={handlePageChange} // Handle page change
        onChangeRowsPerPage={() => {}} // Optional: Handle rows per page change if needed
    />
</section>
                    </main>
                </div>
